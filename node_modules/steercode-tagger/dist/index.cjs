...
// src/patches.ts
var standardHTMLTags = /* @__PURE__ */ new Set([
  "a",
  "abbr",
  "address",
  "area",
...
var r3fElements = /* @__PURE__ */ new Set([
  "Canvas",
  "Box",
  "OrbitControls",
  "Sphere",
...
function isStandardHTMLTag(tagName) {
  return standardHTMLTags.has(tagName.toLowerCase());
}
function shouldTagElement(tagName) {
  if (tagName[0] === tagName[0].toLowerCase())
    return isStandardHTMLTag(tagName);
  return !r3fElements.has(tagName);
}
...
var tailwindJsonOutfile = import_path2.default.resolve(
  projectRoot,
  "./src/tailwind.config.steer.json"
);
var tailwindIntermediateFile = import_path2.default.resolve(
  projectRoot,
  "./.steer.tailwind.config.js"
);
var isSandbox = process.env.STEERCODE_DEV_SERVER === "true";
  ...
  return {
          ...
          enter(node) {
            ...
            if (node.type === "JSXOpeningElement") {
              ...
              magicString.appendLeft(
                jsxNode.name.end ?? 0,
                ` data-steercode-id="${dataComponentId}" data-steercode-name="${elementName}" data-steercode-path="${relativePath}" data-steercode-line="${line}" data-steercode-file="${fileName}" data-steercode-content="${encodeURIComponent(
    ...
    async buildStart() {
      ...
      try {
        ...
        console.error("Error generating tailwind.config.steer.json:", error);
      }
    },
